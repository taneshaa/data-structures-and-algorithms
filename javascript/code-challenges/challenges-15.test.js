...
const screenForNames = (arr) => {
  let names = /^(Dr|Mrs|Ms|Mr). [A-Za-z]+$/;
  let namesFilter = arr.filter(name => names.test(name));
  return namesFilter;
}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
// look at srings in the array
// adjusting the case of letters
...
let biggerThanLuke = (arr) => {
  ...
  return arr.filter(character => parseInt(character.mass) > parseInt(luke.mass)).reduce((str, character, idx, arr) => {
    let delimiter = '';
    if (idx === arr.length - 1) {
      delimiter = ' - ';
    }
